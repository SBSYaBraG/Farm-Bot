 ...
 * 
 * Manages position and state for 3 axes and ALM for 4 active motors
 */
...
// ALM states for all active motors
static MotorALM almStates[MOTOR_COUNT];

// ALM pin mapping
static const uint8_t almPins[MOTOR_COUNT] = {
  ALM_XL_PIN, ALM_XR_PIN, ALM_Y_PIN, ALM_ZL_PIN
  // ALM_ZR_PIN  // Restore with MOTOR_ZR for a dual-Z configuration.
};

static const char* motorNames[MOTOR_COUNT] = {
  "X-Left", "X-Right", "Y", "Z"
  // "Z-Right"  // Restore with MOTOR_ZR for a dual-Z configuration.
};
...
void initializeALM() {
  Serial.println("Initializing ALM monitoring for 4 active motors:");
